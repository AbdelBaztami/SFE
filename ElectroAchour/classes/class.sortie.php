...
class sortie {
	...
	function update() {
		...
		$sql="update sortie set nom='$nom', date=$date, commentaire='$commentaire', ressources='$ressources', coutTTCTotal=$this->coutTTCTotal,Avance=$Avance, nbreArticles=$this->nbreArticles, etat='$this->etat' where idSortie=$this->idSortie";
		executeSql($sql);
	}
	...
	function insert() {
		...
		$date=dateMySql($this->date);
		$Avance=mysqlEscape($this->Avance);
		$sql="insert into sortie (idStock, nom, commentaire, ressources, date, coutTTCTotal,Avance,nbreArticles, etat, corbeille) values ($idStock, '$nom', '$commentaire', '$ressources', $date, $this->coutTTCTotal, $Avance, $this->nbreArticles, '$this->etat', 'N')";
		executeSql($sql);
